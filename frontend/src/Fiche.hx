...
class Fiche implements IJSAsync {
	...
	function bindMoneyActions() {
		...
		mainElem.querySelector(".spells-btn").addEventListener("click", (_) -> {
			new elems.SpellListPopup(character, pushEvent, (p) -> {
				currentSpellPopup = p;
			}, fiche_id);
		});
	}
	...
	function bindMenu() {
		...
		mainElem.querySelector("a.new-day").addEventListener("click", () -> {
			new YesNoAlert("Nouveau jour",
				'Confirmer le début d\'un nouveau jour ? Les sorts préparés seront réinitialisés et le personnage récupère ${character.level} PV.', () -> {
					pushEvent(NEW_DAY);
				});
		});
			...
			if (mod.contains("d")) // It's not a d20
			{
				...
				if (character.basics.characterClass == ROUBLARD && dataParent.dataset.id == "damage") {
					...
					new elems.YesNoAlert("Attaque sournoise ?", 'Ajouter ${sneakDice}d6 de dégâts d\'attaque sournoise ?', () -> {
						...
						new elems.Alert("Attaque sournoise",
							'Dégâts supplémentaires : [${sneakRolls.join(", ")}] = $sneakTotal (total : ${total + sneakTotal})');
					});
				}
				return total;
			}
	...
	private function updateFiche() {
		if (currentSpellPopup != null)
			currentSpellPopup.render();
    ...
    <a class='add-new'>Ajouter un nouvel objet</a>";
		...
		for (item in sorted) {
			...
			li.querySelector(".priority").addEventListener('click', () -> {
				new AmountChoice('Priorité de l\'objet ${item.name}', "Priorité (−1000 à 1000)",
					{defaultValue: if (item.priority == null) 0 else item.priority, canBeNegative: true}, (newPriority, _) -> {
