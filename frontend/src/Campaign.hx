	...
	@:jsasync private function load() {
		...
		for (char in result.fiches) {
			...
			elem.querySelector(".perception").addEventListener("click", () -> {
				...
				if (expMods.length > 0) {
					new elems.ChoicesDialog("Lancer avec modificateur ?", ["Normal"].concat(expMods.map(m -> '${m.why} (${m.mod.asMod()})')), (choice) -> {
						doRoll(if (choice == 0) 0 else expMods[choice - 1].mod);
					});
				} else {
					doRoll(0);
				}
		...
		for (i in 0...encounter.length) {
			...
			switch (entry.entity) {
				case CHARACTER(ficheId):
					fc = charactersByFicheId.get(ficheId);
				case NPC(npcName):
					npc = campaignState.npcs.find(n -> n.name == npcName);
			}

			var name = if (fc != null) fc.basics.characterName else if (npc != null) npc.name else "?";
			var hp = if (fc != null) '${fc.current_hp} / ${fc.getMaxHitPoints()}' else if (npc != null && entry.currentHp != null)
				'${entry.currentHp} / ${npc.maxHp}' else "?";
			var ac = if (fc != null) fc.getAC(NORMAL).string() else if (entry.currentAc != null) entry.currentAc.string() else "?";
			var acTooltip = if (fc != null) 'Contact: ${fc.getACContact()} / Surprise: ${fc.getACSurprise()}' else if (npc != null)
				'Contact: ${npc.acContact} / Surprise: ${npc.acBySurprise}' else "";
			...
			if (isNpc) {
				...
				caCell.addEventListener("click", () -> {
					new elems.AmountChoice("Modifier la CA", "Nouvelle valeur de CA :", {defaultValue: if (entry.currentAc != null) entry.currentAc else 0},
						(newAc, _) -> {
				...
				if (npc != null) {
					...
					saveBtn.addEventListener("click", () -> {
						...
						new ContextMenu(cast saveBtn, [
							...
							if (choice == 0)
								new elems.SkillRollDialog("Vigueur", st.vigor, 0);
							else if (choice == 1)
								new elems.SkillRollDialog("Réflexes", st.reflexes, 0);
							else if (choice == 2)
								new elems.SkillRollDialog("Volonté", st.will, 0);
							return true;
						});
					});
				}
