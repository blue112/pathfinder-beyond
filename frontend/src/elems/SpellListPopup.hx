...
class SpellListPopup extends Popup {
	...
	public function new(character:FullCharacter, pushEvent:FicheEventType->Void, register:(Null<SpellListPopup>) -> Void, ficheId:String) {
		...
		if (character.basics.characterClass == MAGICIEN && character.favoriteMagicSchool == null) {
			...
			new SelectChoicePopup("Choisir une école de magie favorite", schools.map(s -> {value: s.getName(), label: s.spellSchoolToString()}), 1,
		...
		if (character.basics.characterClass == PRETRE && character.priestDomains == null) {
			...
			new SelectChoicePopup("Choisir deux domaines", domainOptions, 2,
	...
	public function render() {
		...
		if (needsPrep && !character.preparationLocked) {
			...
			if (allFull) {
				...
				endBtn.addEventListener("click", () -> {
					new YesNoAlert("Terminer la préparation",
						"Confirmer la fin de la préparation des sorts ? Vous ne pourrez plus modifier vos sorts préparés jusqu'au prochain nouveau jour.",
						() -> {
		...
		var ulClass = if (inPrep && hasPowers) "spell-list-ul has-prep has-available" else if (inPrep) "spell-list-ul has-prep" else if (showAvailable)
			"spell-list-ul has-available" else "spell-list-ul";
		...
		for (spell in sorted) {
			...
			if (needsPrep && character.preparationLocked && !showUnprepared && !isPower) {
				if (!character.lockedPreparedIndices.contains(originalIndex))
					continue;
			}
			...
			if (showAvailable) {
				...
				if (isPower) {
					...
					if (spell.level == 0 && count > 0) {
						...
						if (character.basics.characterClass == PRETRE
							&& spell.level > 0
							&& Rules.getSpontaneousHealSpell(spell.level) != null) {
			...
			if (inPrep && !isPower) {
				...
				minusBtn.addEventListener("click", () -> {
					if (prepCount > 0)
						pushEvent(SPELL_EVENT(UNPREPARE_SPELL(originalIndex)));
				});
				...
				plusBtn.addEventListener("click", () -> {
					...
					if (targetSlot >= 0)
						pushEvent(SPELL_EVENT(PREPARE_SPELL(originalIndex, targetSlot)));
				});
