...
class SpellCastPopup extends Popup {
	...
	static function parseFormula(formula:String, nls:Int):{
		count:Int,
		faces:Int,
		flatMod:Int,
		unparseable:Bool
	...
	public function new(spell:Spell, character:FullCharacter, ficheId:String) {
		...
		if (spell.spellResistance) {
			...
			yesBtn.addEventListener("click", () -> {
				...
				Api.rollDice(ficheId, 20, nls + otherMod, "sort-rm").then(res -> {
					...
					var otherModText = if (otherMod > 0) ' + $otherMod' else "";
					result.innerText = '1d20 (${res.result}) + NLS ($nls)$otherModText = ${res.result + nls}';
		...
		if (spell.savingThrowType != null) {
			...
			var dcStr = if (isPower) {
				if (spell.savingThrowDC != null)
					'DD ${spell.savingThrowDC}'
				else
					"";
			} else {
